function Node(i, j, net) {
  ...
  Node.prototype.show = function() {
    ...
    strokeWeight(1);
    fill(this.color);
    ellipse(this.trans(this.i), this.trans(this.j), node_diameter, node_diameter);
    // Drawing edges
    for (var n = 0; n < this.edges.length; n++) {
      ...
      var dx = this.trans(edge.i);
      var dy = this.trans(edge.j);
      var sx = this.trans(this.i);
      var sy = this.trans(this.j);
      line(dx, dy, sx, sy);
      if (this.showWeightVals) {
        push();
        translate(this.trans(this.i), this.trans(this.j));
    ...
    text("" + nfc(this.value, 2), this.trans(this.i), this.trans(this.j));
  }
  ...
  Node.prototype.mutate = function() {
    ...
    for (var i = 0; i < this.edges.length; i++) {
      if (random() <= this.mr) {
        ...
        if (random() <= this.mr) edge.weight *= -1;
  ...
  Node.prototype.trans = function(p) {
